...
void SkinningObject3dPipeline::Initialize(DirectXCommon* dxCommon) {
	dxCommon_ = dxCommon;
...
void SkinningObject3dPipeline::DrawSetting(BlendMode blendMode) {
	//読み込み済みブレンドモードを検索
...
void SkinningObject3dPipeline::CreateRootSignature() {
	...
	rootParameters[7].DescriptorTable.pDescriptorRanges = &descriptorRange[0];//Tableの中身の配列を指定
...
void SkinningObject3dPipeline::CreateGraphicsPipeline(BlendMode blendMode) {
	//読み込み済みブレンドモードを検索
	...
	//Shaderをコンパイルする
	ComPtr<IDxcBlob> vertexShaderBlob = dxCommon_->CompileShader(L"resources/shaders/SkinningObject3D.VS.hlsl", L"vs_6_0");
	assert(vertexShaderBlob != nullptr);

	ComPtr<IDxcBlob> pixelShaderBlob = dxCommon_->CompileShader(L"resources/shaders/SkinningObject3D.PS.hlsl", L"ps_6_0");
